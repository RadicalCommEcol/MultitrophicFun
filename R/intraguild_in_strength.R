...
#'
#' @examples
intraguild_in_strength <- function(A, sp.names){
  ...
  for(i.sp in 1:nrow(A)){
    if(result$sp[i.sp] %in% sp.names[[1]]$plants){
      result$value[i.sp] <- sum(A[i.sp,plant.pos]) - A[i.sp,i.sp]
    }else if(result$sp[i.sp] %in% sp.names[[1]]$herbivores){
      result$value[i.sp] <- sum(A[i.sp,herb.pos]) - A[i.sp,i.sp]
    }else if(result$sp[i.sp] %in% sp.names[[1]]$floral.visitors){
      result$value[i.sp] <- sum(A[i.sp,fv.pos]) - A[i.sp,i.sp]
    }
